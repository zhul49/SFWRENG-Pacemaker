...
class Login:
    ...
    def __init__(self):
        ...
        self.userStr = StringVar() # Var used to store user input from tk.Entry
        self.passStr = StringVar() # 

        self.userIn = tk.Entry(self.loginWin, relief=FLAT, textvariable=self.userStr) # User input field for username
        self.label1.grid(row=2, column=0, sticky="e")
        self.userIn.grid(row=2, column=1, sticky="w", pady=5)
        self.passIn = tk.Entry(self.loginWin, show="*", relief=FLAT, textvariable=self.passStr) # User input field for password
        ...
        self.loginButton.bind("<Button-1>", self.__validate) # Left-click detection on login button triggers user validation
        self.loginWin.bind("<Return>", self.__validate) # Enables pressing enter to trigger user validation
    ...
    def __validate(self, event):
        ...
        # Opens messagebox on success for correct credentials and proceeds to pacemaker modeSelect screen
        # Opens messagebox on failure for incorrect credentials and refocuses the login window for reattempt
...
class Register:
    # Register class that utilizes the sqlite3 database created by the Database class to add a new user
